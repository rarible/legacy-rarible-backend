  ...
  iter_rp (fun r ->
      if main then
        let name =
          ...
          | None | Some None ->
            let metadata = EzEncoding.destruct Json_encoding.(assoc any_ezjson_value) r#metadata in
            begin match List.assoc_opt "name" metadata with | Some (`String s) -> Some s | _ -> None end
